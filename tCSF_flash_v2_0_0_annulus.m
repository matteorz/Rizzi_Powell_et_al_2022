function fn = tCSF_flash_v2_0_0_annulus(metaParams, graphicParams, stimParams, psychometricParams, eyetrackParams, audioParams)    
...
%         IMG_DIR         = fullfile(RESOURCES_DIR, 'images');
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        p.addParameter('COMMENT',                     	@ischar);
        p.addParameter('ring_x_deg',                    @(x)all(isPositiveNum(x) | isnan(x)));
        p.addParameter('ring_y_deg',                    @(x)all(isPositiveNum(x) | isnan(x)));
        p.addParameter('ring_deg',                      @isnumeric);
        p.addParameter('useRing',                       @(x)ismember(x,[0 1 2]));
        p.parse(stimParams);
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        % Set Annulus Contrast
        % ===================
        %AnnulusContrastRGB = [0,0,0];
        AnnulusContrastRGB = [0.5,0.5,0.5];
        % Set Ring Contrast
        RingContrastRGB = [0.0,0.0,0.0];
        ...
        for i = 1:N
            ...
            stimulusRGB(i,:)  = BackgroundLMS.*(1 + cone_modulation*contrasts(i)); % !!!!!!! Set Contrast
        end
        isValid = all(stimulusRGB>=0 & stimulusRGB<=1, 2);
        idx = find(isValid,1,'last');
        %maxContrast_prcnt = 100*contrasts(idx);
        if idx < N
            %This checks that you're not modulating outside of the range on
            %the top end
            firstInvalidContrast_prcnt = 100*contrasts(idx+1);
        else
            firstInvalidContrast_prcnt = 101;
        end 
        
        %This next step makes sure that your stepsize between contrasts
        %isn't 0 (i.e. no modulation) or smaller than the 256 pallet colour
        %resolution
        isValid2 = all(diff(stimulusRGB(:,coneIdx))>(1./256),2);
        idx2 = find(isValid2,1,'last');
        if isempty(idx2)
            secondInvalidContrast_prcnt = contrasts(1)*100;
        elseif idx2 < N
            secondInvalidContrast_prcnt = contrasts(idx2+1)*100;
        else
            secondInvalidContrast_prcnt = 101;
        end
 
        firstInvalidContrast_prcnt=min(firstInvalidContrast_prcnt, secondInvalidContrast_prcnt);
        ...
        fix_diameter_deg = 0.5;   % Degrees of visual angle (diameter)
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
        ...
        for blockID = 1:nBlocks
            ...
            while ~P.isFinished() % run psychometric block (e.g., 1 staircase)
                ...
                crsPaletteSetPixelLevel(4, RingContrastRGB);
                ...
                if stimParams.useOverlay==2
                    ...
                    %If using rings, draw rings
                    if stimParams.useRing==2
                        crsSetPen1(4);
                        crsSetPen1(4);
                        crsDrawOval(stimParams.ring_x_deg(1),stimParams.ring_y_deg(1), stimParams.ring_deg,stimParams.ring_deg);
                        crsDrawOval(stimParams.ring_x_deg(2),stimParams.ring_y_deg(2), stimParams.ring_deg,stimParams.ring_deg);
                    end
                end
...
%                 Buff_new(2,3) = WhiteRGB(3);
                ...
                Buff_new(4,1) = RingContrastRGB(1); % ensure ring remains visible/invariant
                Buff_new(4,2) = RingContrastRGB(2);
                Buff_new(4,3) = RingContrastRGB(3);
                ...
                %In comments: attemps to get a trigger output - MR.
                %crsObjSetTriggers(CRS.TRIG_TEMPFREQ,0,0);
                % start cycling
                crsSetCommand(CRS.CYCLELUTENABLE);
                %configure trigger out to sync with LUT Cycling system
                %crsSetTriggerOptions(SubSystem,Control,TriggerModeFlags,TriggerLength,Value,MarkerWord,MarkerMask)
                %crsSetTriggerOptions(CRS.TRIGGER_OPTIONS_LUTCYCLE, 0, temporalFrequency_hz, 400, 1, 0, 0);
                %crsSetTriggerOptions(CRS.TRIGGER_OPTIONS_LUTCYCLE, 0, 0, 40, -180, 0, 0);
