...
% <do me>        
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%        
        ...
        if USE_EYETRACKER
            ...
            crsDrawOval(fix_xy_deg,[fix_diameter_deg,fix_diameter_y_deg]);
            % ###
        ...
        for blockID = 1:nBlocks
            ...
            while ~P.isFinished() % run psychometric block (e.g., 1 staircase)
                ...
                if stimParams.useOverlay==2
                    ...
                    blankField = ones(screenDims_px(2)+50, screenDims_px(1)+50)*0.2; % +50 is ahack to make definitely fill the page
                    crsSetPen1(1);
                    crsSetPen2(1);
...
% warning('Provide feedback')                 
                ...
                if stimParams.useOverlay==2
                    crsSetPen1(1);
                    crsSetPen2(1);
                    blankField  = ones(screenDims_px(2)+50, screenDims_px(1)+50)*0.2; % +50 is ahack to make definitely fill the page - 0.5 because in this colourmode 0.5 is black, and 0/1 are white!!!!
                    crsDrawMatrix(blankField);
                    % carve out appertures
                    crsSetPen1(CRS.BACKGROUND);
                    crsSetPen2(CRS.BACKGROUND);
