function fn = tCSF_flash_v2_0_1_annulus_half_half(metaParams, graphicParams, stimParams, psychometricParams, eyetrackParams, audioParams)    
...
%         IMG_DIR         = fullfile(RESOURCES_DIR, 'images');
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        BlueRGB     = [0.0,0.0,1.0];
        BlackRGB    = [1.0,1.0,1.0];
        MaskRGB    = [0.5 0.5 0.5];
        WhiteRGB    = [1.0,1.0,1.0];
        BackgroundRGB = [0.5 0.5 0.5];
        
        % Set Annulus Square Contrast
        % ===================
        %AnnulusContrastRGB = [0.5 0.5 0.5];
        AnnulusContrast_One_RGB = [0.5 0.5 0.5]; %outside/top colour
        AnnulusContrast_Two_RGB = [0 0 0]; %inside/bottom colour
        
        % Set orientation
        % ===================
        orientation = 'vertical'; %options = 'vertical' or 'horizontal', 'checks' 
        checksize = 3; 
        %Checksize is only valid if 'checks' is the option. You have to be
        %able to divide the anulus size (in pixels) into an equal number of
        %checks. With an apperture size of 4.5 (140 px diameter), some
        %valid checksizes include 1,2,5,7,10,70(quarters)
        ...
        fix_diameter_deg = 0.25;   % Degrees of visual angle (diameter)
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
        ...
        for blockID = 1:nBlocks
            ...
            while ~P.isFinished() % run psychometric block (e.g., 1 staircase)
                ...
                % ---------------------------------------------------------
                crsPaletteSetPixelLevel(1, MaskRGB);
                crsPaletteSetPixelLevel(2, BackgroundRGB);
                %crsPaletteSetPixelLevel(3, AnnulusContrastRGB);
                crsPaletteSetPixelLevel(4, [1.0 1.0 1.0]);
                crsPaletteSetPixelLevel(5, [0.0 0.0 0.0]);
                crsPaletteSetPixelLevel(6, AnnulusContrast_One_RGB);
                crsPaletteSetPixelLevel(7, AnnulusContrast_Two_RGB);
                % Display a blank page with fixation cross
                ...
                crsSetSpatialUnits(CRS.PIXELUNIT)
                x_deg = round(crsDegreesToPixels(stimParams.apperture_x_deg));
                y_deg = round(crsDegreesToPixels(stimParams.apperture_y_deg));
                deg = round(crsDegreesToPixels((stimParams.apperture_deg./2)));
                %deg = floor(crsDegreesToPixels((stimParams.apperture_deg./2)));
                
                
                screenDims_px = crsGetScreenSizePixels;
                %screenDims_px = screenDims_px+1500;
                %noise_matrix = zeros(screenDims_px(2)+50, screenDims_px(1)+50); %
                noise_matrix = zeros(screenDims_px(2), screenDims_px(1)) + 1; %
                
                middlepx = screenDims_px./2;
                [sx,sy] = meshgrid(1:screenDims_px(1),1:screenDims_px(2));
                %[x,y] = meshgrid(1:screenDims_px(1)+50,1:screenDims_px(2)+50);
                %checkers = (((-1).^(floor(x/sz)) + (-1).^(floor((y/sz))) == 0));
                checkers = zeros(size(noise_matrix));
                % if diff(x_deg)>r*2
                for side = 1:2
                    cx = x_deg(side)+middlepx(1);
                    cy= y_deg(side)+middlepx(2);
                    ...
                    switch orientation
                        case 'vertical'
                            if side == 1
                                checkers(:,cx-r+1:cx) = 6;
                                checkers(:,cx+1:cx+r) = 7;
                            else
                                checkers(:,cx-r+1:cx) = 7;
                                checkers(:,cx+1:cx+r) = 6;
                            end
                        case 'horizontal'
                            if side == 1
                                checkers(cy-r+1:cy,:) = 6;
                                checkers(cy+1:cy+r,:) = 7;
                            else
                                checkers(cy-r+1:cy,:) = 6;
                                checkers(cy+1:cy+r,:) = 7;
                            end
                        case 'checks'
                            ...
                            checks = zeros(2*r,2*r);
                            
                            for rowpoint = 1:sz*2:size(checks,1)
                                for startpoint = 1:sz*2:size(checks,2);
                                    checks(rowpoint:rowpoint+sz-1,startpoint:startpoint+sz-1) = 6;
                                    checks(rowpoint:rowpoint+sz-1,startpoint+sz:startpoint+(sz*2)-1) = 7;
                                    
                                    checks(rowpoint+sz:rowpoint+(sz*2)-1,startpoint:startpoint+sz-1) = 7;
                                    checks(rowpoint+sz:rowpoint+(sz*2)-1,startpoint+sz:startpoint+(sz*2)-1) = 6;
                                    
                                end
                            end
                            checks = checks(1:(2*r),1:(2*r));
                            checkers(cy-r+1:cy+r,cx-r+1:cx+r) = checks;
                        
                    end
                    
                    %checkers(1:end,cx) = (-1).^(1:size(checkers,1))*-1;
                    %checkers(cy:end,cx) = (-1).^(1:cy+1)*-1;
                    %checkers(checkers==-1)=0;
                    %[x,y]=meshgrid(-(cx-1):(ix-cx),-(cy-1):(iy-cy));
                    %[x,y]=meshgrid(-(cx):(ix-cx-1),-(cy):(iy-cy-1));
                    %c_mask = ((x.^2+y.^2)<=r^2);
                    %c_mask = [cy-r:cy+r;cx-r:cx+r];
                    %c_mask = logical(ones(size(x)));
                    %noise_matrix(c_mask) = checkers(c_mask);
                    
                    %noise_matrix(cy-r+1:cy+r,cx-r+1:cx+r) = checkers(cy-r+1:cy+r,cx-r+1:cx+r);
                    %Plot the annulus ring
                    
                    
                            checkers = padarray(checkers,[1 1],2,'pre');
                            [x,y] = meshgrid(-cx:(ix-cx),-cy:(iy-cy));
                            c_mask = (x.^2 + y.^2) <= r^2;
                            checkers(~c_mask) = 1;
                            %Plot the flashstim ring
                            cxf = round(crsDegreesToPixels(stimParams.eccentricity_x_deg(side)))+middlepx(1);
                            cyf = round(crsDegreesToPixels(stimParams.eccentricity_y_deg(side)))+middlepx(2);
                            flashr = round(crsDegreesToPixels((stimParams.diameter_deg./2)));
                            [x,y] = meshgrid(-cxf:(ix-cxf),-cyf:(iy-cyf));
                            c_mask2 = (x.^2 + y.^2) <= flashr^2;
                            checkers(c_mask2) = 2;
                            checkers = checkers(2:end,2:end);
                            noise_matrix(cy-r+1:cy+r,cx-r+1:cx+r) = checkers(cy-r+1:cy+r,cx-r+1:cx+r);
                ...
                if stimParams.useOverlay==2
                    screenDims_px = crsGetScreenSizePixels;
%                     blankField = ones(screenDims_px(2)+50, screenDims_px(1)+50); % +50 is ahack to make definitely fill the page
%                     crsSetPen1(1);
%                     crsSetPen2(1);
%                     crsDrawMatrix(blankField);
                    ...
                    %crsSetPen1(6);
                    %crsSetPen2(7);
                    %crsSetDrawOrigin(ScreenSize(1)/2 + stimParams.apperture_x_deg(1),ScreenSize(2)/2 +stimParams.apperture_y_deg(1));
                    
                          crsDrawMatrixPalettised(0,0,noise_matrix);
                ...
                % Draw the (blank) flash oval
                for side = 1:2
                    eccentricity_x_deg = stimParams.eccentricity_x_deg(side);
                    eccentricity_y_deg = stimParams.eccentricity_y_deg(side);
...
%                 Buff_new(3,3) = AnnulusContrastRGB(3);
                ...
                Buff_new(6,1) = AnnulusContrast_One_RGB(1); % ensure annulus remains visible/invariant
                ...
                for nLut=1:nFrames
                    ...
                    theNumber=0; % 1000*nFrames;
                    theFrameDelay=1;
                    theFirstLUT=0;
                    theLastLUT=nFrames-1;
                    theLUTStoSkip=1;
                    theStartLUT=1;
                    theTriggerLUT=-1;
                ...
                %[x,y] = meshgrid(1:screenDims_px(1)+50,1:screenDims_px(2)+50);
                %checkers = (((-1).^(floor(x/sz)) + (-1).^(floor((y/sz))) == 0));
                checkers = zeros(size(noise_matrix))+1;
                % if diff(x_deg)>r*2
                for side = 1:2
                    cx = x_deg(side)+middlepx(1);
                    cy= y_deg(side)+middlepx(2);
                    ...
                    iy=screenDims_px(2);
                    r=deg;
                    [blmx,blmy] = meshgrid(0:r*2,0:r*2);
                    switch orientation
                        case 'vertical'
                            if side == 1
                                checkers(:,cx-r+1:cx) = 6;
...
%                     crsDrawMatrix(blankField);
                ...
                eccentricity_x_deg_control = stimParams.eccentricity_x_deg(controlside);
                eccentricity_y_deg_control = stimParams.eccentricity_y_deg(controlside);
                crsSetPen1(2);
                crsDrawOval(eccentricity_x_deg_control,eccentricity_y_deg_control, stimParams.diameter_deg,stimParams.diameter_deg);
                end
...
%                     crsDrawMatrix(blankField);
                ...
                if psychometricParams.useStaticWNoisePostMask
                    ...
                    noise_matrix = round(rand(noise_height_px,noise_width_px));
                    % Create the LUT Object and set parameters
                    ...
                    if ~isnan(stimParams.eccentricity_x_deg(1) + stimParams.eccentricity_y_deg(1))
                        ...
                        crsDrawMatrixPalletised(0,0, noise_matrix);
                    end
                    ...
                    %Otherwise just draw the grey circles again
                                    % Draw the (blank) flash oval
                for side = 1:2
                    eccentricity_x_deg = stimParams.eccentricity_x_deg(side);
                    eccentricity_y_deg = stimParams.eccentricity_y_deg(side);
